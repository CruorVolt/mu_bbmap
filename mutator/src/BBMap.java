...
public final class BBMap extends AbstractMapper {
    ...
    void postparse(String[] args) {
...

        {
            ...
            if (in1 == null && a != null && a.indexOf('=') < 0
                    && (a.startsWith("stdin") || new File(a).exists())) {
                in1 = a;
            }
            if (in2 == null && b != null && b.indexOf('=') < 0
                    && new File(b).exists()) {
                in2 = b;
            }
            if (ERROR_ON_NO_OUTPUT && !OUTPUT_READS && in1 != null) {
                throw new RuntimeException(
                        "Error: no output file, and ERROR_ON_NO_OUTPUT="
                                + ERROR_ON_NO_OUTPUT);
            }
        }
        ...
        if (ambigMode == AMBIG_BEST) {
            ...
            throw new RuntimeException("Unknown ambiguous mapping mode: "
                    + ambigMode);
        }
    ...
    public void setup() {
        ...
        assert (!useRandomReads || maxReads > 0 || (in1 != null && in1
                .equals("sequential"))) : "Please specify number of reads to use.";

        if (minid != -1) {
            MINIMUM_ALIGNMENT_SCORE_RATIO = MSA
                    .minIdToMinRatio(minid, MSA_TYPE);
            sysout.println("Set MINIMUM_ALIGNMENT_SCORE_RATIO to "
                    + String.format("%.3f", MINIMUM_ALIGNMENT_SCORE_RATIO));
        }

        if (!setxs) {
            SamLine.MAKE_XS_TAG = (SamLine.INTRON_LIMIT < 1000000000);
        }
        if (setxs && !setintron) {
            SamLine.INTRON_LIMIT = 10;
        }

        if (outFile == null && outFile2 == null && outFileM == null
                && outFileM2 == null && outFileU == null && outFileU2 == null
                && outFileB == null && outFileB2 == null
                && splitterOutputs == null && BBSplitter.streamTable == null) {
            sysout.println("No output file.");
            OUTPUT_READS = false;
        } else {
            OUTPUT_READS = true;
            if (bamscript != null) {
                BBSplitter.makeBamScript(bamscript, splitterOutputs, outFile,
                        outFile2, outFileM, outFileM2, outFileU, outFileU2,
                        outFileB, outFileB2);
            }
        }
        // assert(false) :
        // bamscript+", "+BBSplitter.streamTable+", "+OUTPUT_READS;

        FastaReadInputStream.MIN_READ_LEN = Tools.max(keylen + 2,
                FastaReadInputStream.MIN_READ_LEN);
        assert (FastaReadInputStream.settingsOK());

        if (build < 0) {
            throw new RuntimeException(
                    "Must specify a build number, e.g. build=1");
        } else {
        ...
        if (ziplevel != -1) {
            ReadWrite.ZIPLEVEL = ziplevel;
        }
        if (reference != null) {
            RefToIndex.makeIndex(reference, build, sysout, keylen);
        }
    }
    ...
    void loadIndex() {
        ...
        if (build > -1) {
            ...
            if (RefToIndex.chrombits != -1) {
                ...
                if (verbose_stats > 0) {
                    sysout.println("Set CHROMBITS to " + RefToIndex.chrombits);
                }
            }
        }

        assert (minChrom >= AbstractIndex.MINCHROM && maxChrom <= AbstractIndex.MAXCHROM) : minChrom
                + ", "
                + maxChrom
                + ", "
                + AbstractIndex.MINCHROM
                + ", "
                + AbstractIndex.MAXCHROM;
        ...
        if (targetGenomeSize > 0) {
            ...
            long x = Tools.max(1,
                    Math.round(0.25f + bases * 1d / targetGenomeSize));
        ...
        if (SLOW_ALIGN || AbstractIndex.USE_EXTENDED_SCORE || useRandomReads
                || MAKE_MATCH_STRING) {
            sysout.println();
            if (INDEX_LOADED) {
                // do nothing
                ...
                assert (RefToIndex.chromlist.size() == maxChrom - minChrom + 1) : RefToIndex.chromlist
                        .size();
        ...
        t.start();
        BBIndex.loadIndex(minChrom, maxChrom, keylen, !RefToIndex.NODISK,
                RefToIndex.NODISK);

        {
            ...
            if (len < 300000000) {
                ...
                if (len < 30000000) {
                    ...
                    BBIndex.MAXIMUM_MAX_HITS_REDUCTION += 1;
                    BBIndex.HIT_REDUCTION_DIV = Tools.max(
                            BBIndex.HIT_REDUCTION_DIV - 1, 3);
        ...
        if (coverageBinned != null || coverageBase != null
                || coverageHist != null || coverageStats != null
                || coverageRPKM != null || normcov != null
                || normcovOverall != null) {
            String[] cvargs = ("covhist=" + coverageHist + "\tcovstats="
                    + coverageStats + "\tbasecov=" + coverageBase + "\tbincov="
                    + coverageBinned + "\tphyscov=" + coveragePhysical
                    + "\t32bit=" + cov32bit + "\tnzo=" + covNzo
                    + "\ttwocolumn=" + covTwocolumn + "\tsecondary="
                    + PRINT_SECONDARY_ALIGNMENTS + "\tcovminscaf="
                    + coverageMinScaf + "\tksb=" + covKsb + "\tbinsize="
                    + covBinSize + "\tstartcov=" + covStartOnly
                    + "\tstrandedcov=" + covStranded + "\trpkm=" + coverageRPKM
                    + "\tnormcov=" + normcov + "\tnormcovo=" + normcovOverall
                    + (in1 == null ? "" : "\tin1=" + in1)
                    + (in2 == null ? "" : "\tin2=" + in2) + (covSetbs ? ("\tbitset="
        ...
        if (!forceanalyze && (in1 == null || maxReads == 0)) {
            return;
        }

        BBIndex.analyzeIndex(minChrom, maxChrom,
                BBIndex.FRACTION_GENOME_TO_EXCLUDE, keylen);
        ...
        // assert(false) :
        // RefToIndex.chrombits+", "+AbstractIndex.CHROMS_PER_BLOCK;
    }
    ...
    public void testSpeed(String[] args) {
        ...
        for (int i = 0; i < mtts.length; i++) {
            try {
                mtts[i] = new BBMapThread(cris, keylen, pileup, SLOW_ALIGN,
                        CORRECT_THRESH, minChrom, maxChrom, keyDensity,
                        maxKeyDensity, minKeyDensity, maxDesiredKeys,
                        REMOVE_DUPLICATE_BEST_ALIGNMENTS, SAVE_AMBIGUOUS_XY,
                        MINIMUM_ALIGNMENT_SCORE_RATIO, TRIM_LIST,
                        MAKE_MATCH_STRING, QUICK_MATCH_STRINGS, rosA, rosM,
                        rosU, rosB, SLOW_ALIGN_PADDING, SLOW_RESCUE_PADDING,
                        OUTPUT_MAPPED_ONLY, DONT_OUTPUT_BLACKLISTED_READS,
                        MAX_SITESCORES_TO_PRINT, PRINT_SECONDARY_ALIGNMENTS,
                        REQUIRE_CORRECT_STRANDS_PAIRS, SAME_STRAND_PAIRS,
                        KILL_BAD_PAIRS, rcompMate, PERFECTMODE,
                        SEMIPERFECTMODE, FORBID_SELF_MAPPING, TIP_SEARCH_DIST,
                        ambiguousRandom, ambiguousAll, KFILTER, IDFILTER,
                        qtrimLeft, qtrimRight, untrim, TRIM_QUALITY,
        ...
        cris.start(); // 4567
        sysout.println("Processing reads in " + (paired ? "paired" : "single")
                + "-ended mode.");
        sysout.println("Started read stream.");

        /*
         * The threads are started after initialization to prevent resource
         * competition between initialization and mapping
         */
        for (int i = 0; i < mtts.length; i++) {
            mtts[i].start();
        }
        sysout.println("Started " + mtts.length + " mapping thread"
                + (mtts.length == 1 ? "" : "s") + ".");
        ...
        printOutput(mtts, t, keylen, paired, false, pileup, scafNzo, sortStats,
                statsOutputFile);
        if (broken > 0 || errorState) {
            throw new RuntimeException(
                    "BBMap terminated in an error state; the output may be corrupt.");
        }
    }
    ...
    void printSettings(int k) {
        ...
        if (verbose_stats >= 2) {
            sysout.println("Key Density:          \t" + keyDensity + " ("
                    + minKeyDensity + " ~ " + maxKeyDensity + ")");
            sysout.println("Max keys:             \t" + maxDesiredKeys);

            sysout.println("Block Subsections:     \t"
                    + BBIndex.CHROMS_PER_BLOCK);
            sysout.println("Fraction To Remove:    \t"
                    + String.format(
                            "%.4f",
                            (BBIndex.REMOVE_FREQUENT_GENOME_FRACTION ? BBIndex.FRACTION_GENOME_TO_EXCLUDE
                                    : 0)));
            // sysout.println("ADD_SCORE_Z:           \t"+Index4.ADD_SCORE_Z);
            sysout.println("Hits To Keep:          \t"
                    + BBIndex.MIN_APPROX_HITS_TO_KEEP);
        }

        if (verbose_stats >= 3) {
            sysout.println("Remove Clumpy:         \t" + BBIndex.REMOVE_CLUMPY);
            if (BBIndex.REMOVE_CLUMPY) {
                sysout.println("CLUMPY_MAX_DIST:       \t"
                        + BBIndex.CLUMPY_MAX_DIST);
                sysout.println("CLUMPY_MIN_LENGTH:     \t"
                        + BBIndex.CLUMPY_MIN_LENGTH_INDEX);
                sysout.println("CLUMPY_FRACTION:       \t"
                        + BBIndex.CLUMPY_FRACTION);
            }
            sysout.println("Remove Long Lists:     \t"
                    + BBIndex.TRIM_LONG_HIT_LISTS);
            if (BBIndex.TRIM_LONG_HIT_LISTS) {
                sysout.println("HIT_FRACTION_TO_RETAIN:\t"
                        + BBIndex.HIT_FRACTION_TO_RETAIN);
            }
            sysout.println("Trim By Greedy:        \t" + BBIndex.TRIM_BY_GREEDY);
            sysout.println("Trim By Total Sites:   \t"
                    + BBIndex.TRIM_BY_TOTAL_SITE_COUNT);
            if (BBIndex.TRIM_BY_TOTAL_SITE_COUNT) {
                sysout.println("MAX_AVG_SITES:         \t"
                        + BBIndex.MAX_AVERAGE_LIST_TO_SEARCH);
                sysout.println("MAX_AVG_SITES_2:       \t"
                        + BBIndex.MAX_AVERAGE_LIST_TO_SEARCH2);
                sysout.println("MAX_SHORTEST_SITE:     \t"
                        + BBIndex.MAX_SHORTEST_LIST_TO_SEARCH);
            }
            sysout.println("Index Min Score:       \t" + BBIndex.MIN_SCORE_MULT);

            sysout.println("Dynamic Trim:          \t"
                    + BBIndex.DYNAMICALLY_TRIM_LOW_SCORES);
            if (BBIndex.DYNAMICALLY_TRIM_LOW_SCORES) {
                sysout.println("DYNAMIC_SCORE_THRESH:  \t"
                        + BBIndex.DYNAMIC_SCORE_THRESH);
            }
        }
