 ...
 *
 */
public class TableLoaderLockFreeU {
	...
	public static void main(String[] args){
		...
		AbstractKmerTableU[] tables=makeTables(AbstractKmerTableU.ARRAY1D, initialSizeDefault, true);
	...
	public TableLoaderLockFreeU(AbstractKmerTableU[] tables_, int k_){
		this(tables_, k_, 0, 0, 0, 0, true, false);
	}
	
	public TableLoaderLockFreeU(AbstractKmerTableU[] tables_, int k_, int mink_, int speed_, int hdist_, int edist_, boolean rcomp_, boolean maskMiddle_){
		tables=tables_;
		k=k_;
		k2=k-1;
		mink=mink_;
	...
	public static AbstractKmerTableU[] makeTables(int tableType, int initialSize, boolean growable){
		return AbstractKmerTableU.preallocate(WAYS, tableType, initialSize, growable);
	}
...
//				long extraBase=(i>=bases.length-1 ? -1 : AminoAcid.baseToNumber[bases[i+1]]);
		...
		private final AbstractKmerTableU map;
