...
public class TableReaderU {
	...
	public static void main(String[] args){
		...
		AbstractKmerTableU[] tables=TableLoaderLockFreeU.makeTables(AbstractKmerTableU.ARRAY1D, 128000, true);
		...
		TableLoaderLockFreeU loader=new TableLoaderLockFreeU(tables, k, mink, speed, hdist, edist, rcomp, maskMiddle);
		loader.setRefSkip(0);
		loader.hammingDistance2=0;
		loader.editDistance2=0;
		loader.storeMode(TableLoaderLockFreeU.SET_IF_NOT_PRESENT);
		...
		int qskip=0;
		int qhdist=0;
		TableReaderU tr=new TableReaderU(k, mink, speed, qskip, qhdist, rcomp, maskMiddle);
		
		//TODO: Stuff...
	}
	
	public TableReaderU(int k_){
		this(k_, 0, 0, 0, 0, true, false);
	}
	
	public TableReaderU(int k_, int mink_, int speed_, int qskip_, int qhdist_, boolean rcomp_, boolean maskMiddle_){
		k=k_;
		k2=k-1;
		mink=mink_;
		rcomp=rcomp_;
	 ...
	 */
	public final int kMask(final Read r, final AbstractKmerTableU[] sets){
		if(r==null){return 0;}
	 ...
	 */
	public final int countKmerHits(final Read r, final AbstractKmerTableU[] sets){
	 ...
	 */
	public final int findBestMatch(final Read r, final AbstractKmerTableU[] sets){
		idList.size=0;
	 ...
	 * @return Number of bases masked
	 */
	public final BitSet markBits(final Read r, final AbstractKmerTableU[] sets){
	 ...
	 */
	public final int getValue(final Kmer kmer, final int len, final int qHDist, final int qPos, final AbstractKmerTableU[] sets){
		if(qSkip>1 && (qPos%qSkip!=0)){return -1;}
		return qHDist<1 ? getValue(kmer, len, sets) : getValue(kmer, len, qHDist, sets);
	}
	 ...
	 * @return Value stored in table, or -1
	 */
	public final int getValue(final Kmer kmer, final int len, final int qHDist, final AbstractKmerTableU[] sets){
	 ...
	 */
	public final int getValue(final Kmer kmer, final int len, final AbstractKmerTableU[] sets){
	 ...
	 * @return Value stored in table
	 */
	public final int getValueWithMask(final Kmer kmer, final long lengthMask, final AbstractKmerTableU[] sets){
		...
		if(noAccel || ((key/WAYS)&15)>=speed){
			...
			AbstractKmerTableU set=sets[(int)(key%WAYS)];
	 ...
	 */
	private final long toValue(final Kmer kmer){
