...
//		assert(false) : rnaArtifactFlag+"\n"+primaryArgList+"\n"+libType+"\n"+outDir;
	...
	public void process(){
		...
		if(fileListName!=null){
			...
			if(ihistName!=null){sb.append("ihist="+(ihistName.startsWith(outDir) ? ihistName.substring(xlen) : ihistName)).append('\n');}
			if(scaffoldStatsName!=null){sb.append("scafstats="+(scaffoldStatsName.startsWith(outDir) ? scaffoldStatsName.substring(xlen) : scaffoldStatsName)).append('\n');}
			if(refStatsName!=null){sb.append("refstats="+(refStatsName.startsWith(outDir) ? refStatsName.substring(xlen) : refStatsName)).append('\n');}
			if(riboFlag && riboOutFile!=null){sb.append("ribo="+(riboOutFile.startsWith(outDir) ? riboOutFile.substring(xlen) : riboOutFile)).append('\n');}
		...
		{
			int step=0;
			final int numSteps=(doFilter ? 1 : 0)+(doTrim ? 1 : 0)+(doNextera ? 1 : 0)+(riboFlag ? 1 : 0)+((humanFlag || catDogHumanFlag) ? 1 : 0)+mappingRefs.size();
...
//				System.err.println("Filter. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				...
				filter(in1z, in2z, out1z, out2z, inPrefix, outPrefix, step);
...
//				System.err.println("Human. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				...
				dehumanize(in1z, in2z, out1z, out2z, inPrefix, outPrefix, step, catDogHumanFlag);
			...
			if(mappingRefs.size()>0){
				for(int i=0; i<mappingRefs.size(); i++){
					...
					decontamByMapping(in1z, in2z, out1z, out2z, inPrefix, outPrefix, mappingRefs.get(i), step);
...
//				System.err.println("Nextera. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				...
				if(doMerge){merge(in1z, in2z, inPrefix, step);}
...
//				System.err.println("Merge. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				if(step==1){
					merge(in1, in2, null, step);
				}else{
					merge(out1, out2, null, step);
				}
			}
		}
...
//				refs.add(clipLinker);
	 ...
	 * @param outPrefix Append this prefix to output filenames
	 */
	private void filter(String in1, String in2, String out1, String out2, String inPrefix, String outPrefix,
			int stepNum){
...
//			if(rqcStatsName!=null){al.add("rqc="+rqcStatsName);} //Old style for 2 log files
	 ...
	 * @param outPrefix Append this prefix to output filenames
	 */
	private void dehumanize(String in1, String in2, String out1, String out2, String inPrefix, String outPrefix,
	 ...
	 * @param outPrefix Append this prefix to output filenames
	 */
	private void decontamByMapping(String in1, String in2, String out1, String out2, String inPrefix, String outPrefix,
	 ...
	 */
	private void merge(String in1, String in2, String prefix, int stepNum){
...
//			if(in1!=null){temp.add("in1="+inPre+in1);}
//			if(in2!=null){temp.add("in2="+inPre+in2);}
//			if(cardinalityName!=null){temp.add("out="+cardinalityName);}
//			String[] loglogArgs=temp.toArray(new String[0]);
//			jgi.LogLog.main(loglogArgs);
//			if(reproduceName!=null){
//				writeReproduceFile(reproduceName, "loglog.sh", loglogArgs, true, false, false);
//			}
//		}
...
//		sdf.setTimeZone(TimeZone.getTimeZone("PST"));
	...
	private String riboKmers = "/global/projectb/sandbox/gaag/bbtools/ribo/fusedKmers10.fa.gz";
