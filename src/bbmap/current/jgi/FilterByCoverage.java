...
//			outstream.println("Fetched "+reads);
			...
			while(reads!=null && reads.size()>0){
				...
				for(int idx=0; idx<reads.size(); idx++){
					...
					if(trimEnds>0){
						if(initialLength1-trimEnds*2<Tools.max(minLength, 1)){
					...
					if(csl1!=null){
						...
						if(csl0!=null){
							...
							if(csl1.reads()<minReads || csl1.length<minLength || csl1.coveredPercent()<minCoveredPercent || csl1.avgFold<minCoverage){
								contam=true;
							}else if(basesUnderMin>0 && underMin>basesUnderMin){
								contam=true;
							}else{
